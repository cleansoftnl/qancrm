...
@section('role-form')
  ...
  <div class="alert alert-info">
    <p><strong>Warning:</strong> This panel will show any permission groups that has been specified. However, it will
      not show any permissions that have a resource_id attached.</p>
  </div>
          ...
          @foreach($tier_one as $module)
            ...
            @else
              <li>
                ...
                <a href="#t1_{{ $module }}" data-toggle="pill">{{ ucwords($module) }} <span
                    class="badge pull-right">{{ count($tier_two[$module]) }}</span></a>
              </li>
              @endforeach
        </ul>
      </div>
                          ...
                          @foreach($tier_two[$group] as $model)
                            ...
                            @else
                              <li>
                                ...
                                <a href="#t2_{{ $group.'_'.$model }}" data-toggle="pill">{{ ucwords($model) }} <span
                                    class="badge pull-right">{{ count($permissions->where('resource_type', $group.'_'.$model)->where('resource_id', NULL)) }}</span></a>
                              </li>
                              @endforeach
                        </ul>
                      </div>
        ...
        (function ($) {
          $('select.master-select').on('change', function () {
            ...
            $(this)
              ...
              .change();
            /* trigger a change to make it update */
          });
        })(jQuery);
      </script>
@stop
